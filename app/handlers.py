...
async def login(user_form: UserLoginForm = Body(..., embed=True), database=Depends(connect_db)):
...
async def create_user(user: UserCreateForm = Body(..., embed=True), database=Depends(connect_db)):
